...
const ListContainer = () => {
  ...
  const perPageHandler = (perPage: number) => {
    ...
    let lastPageNumber = Math.ceil(setProducts.length / perPage);
  ...
  useEffect(() => {
    if (products) {
      ...
      const pageValue = getSearchParams("page");
      if (!pageValue) {
  ...
  }, [products, setSearchParams, getSearchParams]);
